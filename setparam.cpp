...
const char* Werte::kenntext(int nummer) {
   ...
   break;
      ...
      case 226: {
         return "Define fixed glucose gradient maximum value (mM)";
      }
      break;
   ...
   //Jiaojiao: read the parameter about bcr, cd40, polarity of TFs, affinity-based, BLIMP1 threshold.
   case 900: {
   ...
   break;
   case 906: {
     return "BLIMP1 threshlod for plasmacell differentiation";
   }
   break;
   //Jiaojiao: end
...
void Werte::fGet(char * parname, bool transform2rate) {
   ...
   // "Sequence Space:\n";
   // Jiaojiao: read bcr intensity, cd40 intensity, decision rule, polarity parameters or BLIMP threshold.
   if (fFind(parname, s, 900) == 1) {
      s >> bcr;
   }
   if (fFind(parname, s, 901) == 1) {
      s >> cd40;
   }
   if (fFind(parname, s, 902) == 1) {
      s >> pBLIMP1;
   }
   if (fFind(parname, s, 903) == 1) {
      s >> pIRF4;
   }
   if (fFind(parname, s, 904) == 1) {
      s >> pBCL6;
   }
   if (fFind(parname, s, 905) == 1) {
      s >> type_CD40_signal;
   }
   if (fFind(parname, s, 906) == 1) {
      s >> BLIMP1th;
   }
   //Jiaojiao: end
